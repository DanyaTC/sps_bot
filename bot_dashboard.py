...
# Определяем макет
app.layout = html.Div([
    html.H1('Состояние подачи заявок на разработку ПО', style={'textAlign': 'center'}),
    dcc.Dropdown(
        id='date-dropdown',
        options=[
            {'label': 'Месяц', 'value': 'ME'},
...
def update_figures(selected_date):
    ...
    time_series_figure = px.line(filtered_df, x='Date', y='Priority (%)', title='График временного ряда')

    # Создание круговой диаграммы с помощью Plotly
    pie_chart_figure = px.pie(df, names='Priority (dis)', title='Круговая диаграмма')

    # Создание гистограммы с помощью Plotly
    histogram = px.histogram(df, x='Priority (dis)', title='Гистограмма')

    # Создание графика рассеивания с помощью Plotly
    scatter_plot_figure = px.scatter(df, x='Priority (%)', y='Date', title='График рассеяния')
